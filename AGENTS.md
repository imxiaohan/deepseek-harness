...
- **Product UI copy is locale-owned.** Browser text uses typed dictionaries and `t` or localized props; native text uses typed app-locale dictionaries; `verify-client-ui-i18n` rejects hardcoded copy ([decision](.agents/notes/implemented/architecture/2026-08-23-locale-owned-client-ui-copy.md)).
