...
- **ConnectionController**（位于 `packages/client/connection`）：打开 `$events` Remote 流、通过 for-await 泵入，并在 generation 围栏内指数退避重连（500ms 翻倍至 10s 封顶、抖动、无限重试）；sink 单向注入，Controller 不认识 Session。重连即重建：`onConnected` → 列表刷新 + 各已打开会话 resync。对象层通过 `ctx.remote` 调用生成的命名空间；Web 以 HTTP POST 承载 Remote 一元调用、以 API Gateway 的 WebSocket mux 承载逻辑流，Desktop 则通过 IPC 提供等价的 fetch 与 stream hook。Connection 在两种载体上都拥有请求传输与 generation。
