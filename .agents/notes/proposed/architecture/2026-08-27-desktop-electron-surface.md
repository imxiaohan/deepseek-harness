...
- M2, the desktop bundle and IPC carrier: the custom scheme, the preload transport, and the dropped webserver rows; the surface stops listening on any port. Shipped shape: the tree stays in the milestone-1 host child; renderer fetches and stream frames relay through the Electron main process (message-based until latency evidence demands `MessageChannelMain` ports); the preload installs the hooks with `contextIsolation: false` so `Response` values cross by reference, the carrier's trust line being the main process's sender gate; and the virtual `webServer`'s `host` getter returns the synthesized loopback authority, because bind-dependent consumers (the directory picker's boot sampling) read it — `port` throws.
