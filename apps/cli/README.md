...
| `dsh desktop` | Launch the Electron application; its host child boots the `desktop` profile. |
| `dsh plugin --profile <name> <pnpm args>` | Manage a profile's plugins by forwarding to pnpm in the profile directory. |

The invoking directory is the default workspace root. `dsh desktop` launches Electron, while `dsh --profile desktop` boots only the host profile for the Electron child, composition tests, and recorded-session adapters. The `web`, `desktop`, `headless`, `sdk`, `sdk-minimal`, and `acp` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`.
