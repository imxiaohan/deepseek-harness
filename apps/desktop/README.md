...
The CLI starts its installed `@deepseek-ai/dsh-desktop` assembly and passes `--patch` overlays and remaining app arguments to the host child in a validated process-launch envelope; direct `dsh --profile desktop` runs only that host profile and does not create a window. The preload installs `window.__DSH_TRANSPORT__ = { fetch, openStream, ownsHost: true }` before any client plugin loads and projects the presenter's `light`, `dark`, or `system` theme source onto Electron's native window chrome. The composed directory picker routes each pick over the carrier in reverse — one correlated host→main request, the main process opens `dialog.showOpenDialog` from the shell window, and the chosen path returns to the host child. Stored credentials sit behind the operating system's keychain instead: the main process owns one `safeStorage`-encrypted document under the app data directory, and the composed credentials provider reaches it through the same native lane while keeping the environment and `.env` layers of the file-backed provider. External `dsh://open?path=` links validate completely in main — malformed, unsupported, and cross-authority input (including the internal `dsh-desktop://` scheme) is refused before anything it names reaches a command or host API — and the accepted intent focuses the window and adopts the workspace through the existing `workspace/create` API over the carrier; registration of the public scheme happens only in packaged builds. The shell runs `contextIsolation: false` so the connection client receives those hooks by reference, but accepts carrier IPC and custom-scheme host routes only from the window's current main frame while it remains on the loaded authority; reload cancels work owned by the outgoing document, and cross-authority navigation and child windows are denied. Custom-scheme response bodies cross the process channel one pull-driven binary chunk at a time, so Session export does not materialize a complete archive in the host or main process. Quit revokes document admission, cancels renderer work, sends a platform-independent shutdown message, and waits for the host bridge and profile tree to reach quiescence; an unconditional process kill ends a child that exceeds the bounded grace period. The shell rejects a second launch while one instance is active and reports fatal conditions through the console and locale-owned system notification and error-dialog copy before exiting nonzero.

## Known Limitations and Deferred Work

- Native capability providers (notification answers) and distribution hardening remain proposed; see the [native integration and distribution note](../../.agents/notes/proposed/architecture/2026-08-27-desktop-native-integration-and-distribution.md). The Electron directory picker, the safeStorage credential store, and validated application deep links are implemented.
