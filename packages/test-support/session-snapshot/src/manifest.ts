...
export type SnapshotProfile = 'headless' | 'desktop' | 'sdk' | 'acp' | 'web'
...
const PROFILES = new Set<SnapshotProfile>(['headless', 'desktop', 'sdk', 'acp', 'web'])
...
export function parseSnapshotManifest(source: string, path = 'snapshot.yml'): SnapshotManifest {
  ...
  try {
    ...
    if (typeof root.profile !== 'string' || !PROFILES.has(root.profile as SnapshotProfile)) {
      throw new Error('manifest.profile must be headless, desktop, sdk, acp, or web')
    }
