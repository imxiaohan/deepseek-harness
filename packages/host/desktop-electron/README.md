...
| [`src/native-host-client.ts`](src/native-host-client.ts) | The host-child native-op client: correlated native round trips to Electron main |
...
The host bridge publishes the boot payload once its message listener is installed; the main process does not create the scheme-loaded window before receiving it. Both process adapters validate every carrier message and terminate their process on invalid fields instead of leaving boot, fetch, or stream work pending. Fetch and stream cancellation crosses both IPC hops; the host bridge owns each response-body reader, and bridge disposal cancels those readers and awaits every active operation before profile teardown completes. Plugin combo URLs keep their complete pathname and query string through the fetch protocol and run through the virtual server's registered `/plugins` handler; custom-scheme `/api` requests, including Session export downloads, use the shared API Fetch handler after Electron main authorizes the document. Scheme response bodies advance by one binary chunk per main-process pull, so the process channel and its endpoints never materialize a complete Session archive. The virtual `webServer`'s `host` getter returns the loopback authority used by host-side URLs, so bind-dependent consumers pick their loopback branch; `port` throws, because the desktop composition listens on no port. Host-initiated native operations ride the same carrier in the reverse direction: `DesktopRuntime.nativeRequest` sends one correlated `native-request` carrying a closed-vocabulary op and its validated args, Electron main performs the operation — the OS directory chooser, the `safeStorage`-backed credential store — and answers with `native-ok` or `native-error`. The lane binds lazily to the host child's process channel so boot-time consumers cross before the post-boot carrier wiring runs; the [directory-picker](../directory-picker-electron/README.md) and [credentials](../../credentials/credentials-electron/README.md) providers are its seam consumers.
